...
def new_fast_gpt(cov,EF_old,X_dom,old_pts,new_pts,loc_NP,SN_without_der,P_without_der,X_test,rhs_with_der, sol_init,nugget,rho=4.0,k_neighbors=1,N_threads=1,lamb=1.5,alpha=1.0):
    N=np.shape(X_dom)[1]
    ...
    if args.type=="points":
        ...
        print("Type not supported in dynamic update")
...
if __name__=="__main__":
    ...
    der_indices=[[0], [1],[2],[1,1],[1,2],[2,2]] # Indexing used for reading derivatives
    ...
    def run_fast_cg(pa):
            ...
            sol_1,temp_EF,temp_SN, temp_P=fast_gpt_cg(cov,data,data_test, truth_with_der,sol_init,nugget,rho=rho,k_neighbors=k_neighbors,N_threads=args.threads,lamb=1.5,alpha=1.0)
    ...
    sol_1,EF,SN_without_der,P_without_der =fast_gpt_cg(cov,data,data_test, truth_with_der,sol_init,nugget,rho=rho,k_neighbors=k_neighbors,N_threads=args.threads,lamb=1.5,alpha=1.0)
    ...
    for i in range(new_size):
        ...
        if report[0]['is_outlier']==True or err_ratio_hit>=3 or budget_count>=data_cost_budget:
            ...
            def run_fast_cg_retrain(pa):
                len_sc,nugget=pa
                cov = GaussianCovariance_generic(len_sc)
                sol_1,temp_EF,temp_SN, temp_P=fast_gpt_cg(cov,data_new,data_test, truth_with_der,sol_init,nugget,rho=rho,k_neighbors=k_neighbors,N_threads=args.threads,lamb=1.5,alpha=1.0)
                ac=np.mean((sol_1-test_truth)**2)
                return ac

            # Function with rho a parameter for optimzation
            ...
            if opti_err > check_err:
                ...
                if not break_flag:
                    coun=0
                    cov = GaussianCovariance_generic(lengthscale)
                    sol_1,EF,new_SN_wot_der,new_P_wot_der =fast_gpt_cg(cov,data_new,data_test, truth_with_der,sol_init,nugget,rho=rho,k_neighbors=k_neighbors,N_threads=args.threads,lamb=1.5,alpha=1.0)
            ...
            sol_1_new,EF_new,new_SN_wot_der,new_P_wot_der=new_fast_gpt(cov,EF,data_new,X_domain.T,new_points[i:i+1,:].T,
                                                                                            loc_np,SN_without_der1,P_without_der,data_test, truth_with_der,
    ...
    err_all.append(err)
    Sp_all.append(sp_)
    np.savetxt(f"./results/dynamic_2D_2_approrach_1_err.csv",err_all,delimiter=",")
